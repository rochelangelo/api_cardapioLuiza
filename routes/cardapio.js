...
router.post("/", async (req, res, next) => {
  try {
    ...
    if (produto.nome != "" && produto.valor > 0 && produto.descricao != "" && produto.opcionais != "" && produto.categoria != "") {
      if (produto.valorOpcional > 0) {
        ...
        res.status(200).redirect('http://localhost:3000/').send({ message: "O item " + produto.nome + " foi cadastrado!" });

      } else if (produto.opcionais === "não tem" && produto.valorOpcional === 0) {
        produto = {
          id: data.nextId++,
          nome: produto.nome,
          valor: produto.valor,
        ...
        logger.info(`POST /produto - ${JSON.stringify(produto.id)}:${JSON.stringify(produto.nome)}`);
        res.status(200).redirect('http://localhost:3000/').send({ message: "O item " + produto.nome + " foi cadastrado!" });
...
router.get("/:id", async (req, res, next) => {
  try {
    const data = JSON.parse(await readFile("./dados/cardapio.json"));
    const produto = data.produtos.find(produto => produto.id === parseInt(req.params.id));
    if (produto != null) {
      res.send(produto);
      logger.info(`GET /produto - ${JSON.stringify(produto.id)}:${JSON.stringify(produto.nome)}`);
    } else {
...
router.delete("/:id", async (req, res, next) => {
  try {
    const data = JSON.parse(await readFile("./dados/cardapio.json"));
    data.produtos = data.produtos.filter(produto => produto.id !== parseInt(req.params.id));
    await writeFile("./dados/cardapio.json", JSON.stringify(data));
    logger.info(`DELETE /produto - ${JSON.stringify(req.params.id)}`);
    res.status(200).send({ message: "O item " + req.params.id + " foi DELETADO!" });
  } catch (err) {
    next(err);
  }
});
...
router.put("/", async (req, res, next) => {
  try {
    ...
    await writeFile("./dados/cardapio.json", JSON.stringify(data, null, 2));
    logger.info(`PUT /produto - ${JSON.stringify(produto.id)}:${JSON.stringify(produto.nome)}`);
    res.status(200).send({ message: "O item " + produto.id + ":" + produto.nome + " foi ALTERADO!" });
  } catch (err) {
    next(err);
  }
});
